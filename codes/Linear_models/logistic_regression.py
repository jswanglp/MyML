...
# #@title Logistic Reression { display-mode: "both" }
# coding:utf-8
import os
import numpy as np
...
if __name__ == '__main__':
    ...
    with graph.as_default():
        ...
        acc_sum = tf.summary.scalar('acc_summary', accuracy, ['train', 'test']) # 关于accuracy的summary
    ...
    with tf.Session(graph=graph) as sess:
        ...
        #                                                 feed_dict={image_place: image, label_place: label})
            ...
            if (train_loss <= min_cross) & (train_acc > max_acc): # 按照要求保存网络模型
